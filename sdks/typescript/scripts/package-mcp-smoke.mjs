...
const tmp = mkdtempSync(join(tmpdir(), "sello-mcp-package-test-"));
...
try {
  ...
  writeFileSync(join(projectDir, "mcp-smoke.mjs"), mcpSmokeSource());

  run(npmCommand, ["install", "--ignore-scripts", packageSpec], { cwd: projectDir });
  run(process.execPath, ["mcp-smoke.mjs"], { cwd: projectDir });

  console.log(`MCP package smoke test passed for ${packageSpec}.`);
} catch (error) {
  failed = true;
  console.error(`MCP package smoke test temp directory: ${tmp}`);
  throw error;
} finally {
  if (!failed) {
...
const serviceKid = textEncoder.encode("mcp-smoke-service-key");
...
assert.equal(response.content[0].text, "created Launch checklist");
...
assert.equal(result.receipts[0].receipt["action-type"], "mcp.tools/call.calendar.create_event");
